...
rasterizeGaussiansFeaturesCUDA(
    ...
    torch::Tensor out_features = torch::full({channels, H, W}, 0.0, float_opts);
    ...
    return std::make_tuple(rendered, out_features, radii, geomBuffer, binningBuffer, imgBuffer);
}
...
rasterizeGaussiansFeaturesBackwardCUDA(
    ...
    const int channels = features.size(1);
    const int P = means3D.size(0);
    const int H = dL_dout_features.size(1);
    const int W = dL_dout_features.size(2);

    torch::Tensor dL_dmeans3D = torch::zeros({P, 3}, means3D.options());
    torch::Tensor dL_dmeans2D = torch::zeros({P, 3}, means3D.options());
    torch::Tensor dL_dfeatures = torch::zeros({P, channels}, means3D.options());
